			...
			<div id="detailed-panel">
				<table>
					...
					<tr>
						...
						<th colspan="6" style="width:180px">大小比走势</th>
					</tr>
					<tr>
						<th>01</th>
						<th>02</th>
	...
	<script type="text/javascript">
		...
		$(function()
		{
			...
			({
				...
				clickitem:function()
				{
					window.location.href = '4.jsp?type='+$("#11x5-type").val()+'&length='+$("#11x5-length").val();
				}
			});
			...
			$.getJSON("../service/data.jsp?mode=1&length="+$("#11x5-length").val()+"&type="+$("#11x5-type").val(), function(response)
			{
				if(response.status == 1)
				{
					...
					for(var i = 10 ; i < rows.length ; i++)
					{
						...
						content += '<td class="D1">'+(codes[0] > 5 ? '<span class="greenrect">大</span>' : '')+'</td>';
						content += '<td class="X1">'+(codes[0] <= 5 ? '<span class="greenrect">小</span>' : '')+'</td>';
						content += '<td class="D2">'+(codes[1] > 5 ? '<span class="yellowrect">大</span>' : '')+'</td>';
						content += '<td class="X2">'+(codes[1] <= 5 ? '<span class="yellowrect">小</span>' : '')+'</td>';
						content += '<td class="D3">'+(codes[2] > 5 ? '<span class="bluerect">大</span>' : '')+'</td>';
						content += '<td class="X3">'+(codes[2] <= 5 ? '<span class="bluerect">小</span>' : '')+'</td>';
						content += '<td class="D4">'+(codes[3] > 5 ? '<span class="purplerect">大</span>' : '')+'</td>';
						content += '<td class="X4">'+(codes[3] <= 5 ? '<span class="purplerect">小</span>' : '')+'</td>';
						content += '<td class="D5">'+(codes[4] > 5 ? '<span class="seagreenrect">大</span>' : '')+'</td>';
						content += '<td class="X5">'+(codes[4] <= 5 ? '<span class="seagreenrect">小</span>' : '')+'</td>';
